...
Readium.Views.FilePickerView = Backbone.View.extend({
	...
	initialize: function() {
		...
		$(this.el).on('shown', function(){
			...
			$.get( url, function( resultList ) {
				if(resultList){
					bookList ='	<table class="table table-hover"><thead><tr><th>#</th><th>Fiile Name</th><th>Size</th><th>Action</th></tr></thead><tbody>';
					for (i = 0; i < resultList.length; i++) { 
						bookList += '<tr><td>' + (i + 1) + '</td><td>&nbsp;' + resultList[i].title + '</td><td>' + resultList[i].fileSize/1000000 + '&nbsp;Mb</td><td><a durl="'+resultList[i].downloadUrl+'&access_token='+resultList[i].token+'" type="button" class="buttonsGdrive btn btn-info">Add Book</a>&nbsp;&nbsp;&nbsp;</td></tr>';
					}
					bookList += '</tbody></table>';
				}else{
					bookList += 'Your Google session expired, Please relogin!';
				}
				$('#gDriveFiles').html(bookList); 
			});
			
			//End gDrive file listning
			$('#add-book-heading').focus();
			setTimeout( function() {
				$('#add-book-btn').attr('aria-pressed', 'true');
			}, 1);
	...
	handleUrl: function(evt) {
		evt.preventDefault();
  		var gDriveDwnUrl = $(evt.currentTarget).attr('durl');
		var that = this;
		var input = document.getElementById('book-url');
		if(gDriveDwnUrl === null || gDriveDwnUrl.length < 1) {
			alert("invalid url, cannot process");
		}
		else {
			var url = gDriveDwnUrl;			
			...
			xhr.onload = function() {
				if (this.status == 200) {
					...
					var extractor = new Readium.Models.ZipBookExtractor({file: blob, src_filename: 'aaaa.epub'});
